...
class _CancelledBookingScreenState extends State<CancelledBookingScreen> {
  ...
  // Previous booking IDs for new-booking alert detection
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      if (response.content != null) {
        final canceledList = response.content
            .where((b) {
        ...
        if (_knownBookingIds.isNotEmpty) {
          final newBookings = canceledList
        ...
        final newIds = canceledList.map((b) => b.bookingRef).toSet();
        _knownBookingIds = newIds;

        setState(() {
          _bookings = canceledList;
          _totalElements = canceledList.length;
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  void _showNewBookingAlert(BookingModel booking) {
    ...
    final schedDate = _formatScheduleDate(booking.bookingDate);
              ...
              children: [
                Row(children: [
                  Container(
                    ...
                    child: const Icon(Icons.notifications_active_rounded, color: Color(0xFFEF7822), size: 22),
                  ),
  ...
  void _handleDownload() {
    ...
    try {
      ...
      final fileName =
          'canceled_bookings_${DateFormat('yyyyMMdd_HHmm').format(DateTime.now())}.xlsx';
