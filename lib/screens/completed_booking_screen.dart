...
class _CompletedBookingScreenState extends State<CompletedBookingScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      if (response.content != null) {
        ...
        final newIds = offlineList.map((b) => b.bookingRef).toSet();
        _knownBookingIds = newIds;

        setState(() {
          _bookings = offlineList;
          _totalElements = offlineList.length;
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  void _handleDownload() {
    ...
    try {
      ...
      final sheet = excel['Completed Bookings Report'];
      ...
      final fileName =
          'completed_bookings_${DateFormat('yyyyMMdd_HHmm').format(DateTime.now())}.xlsx';
